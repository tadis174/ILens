...
public static class FindMethodsTool
{
    ...
    public static string FindMethods(
        ...
        [Description("Optional return-type pattern (e.g., 'bool', 'IEnumerable', 'Pawn').")] string returns = null,
        ...
        [Description("Optional exact declaring namespace, e.g. 'RimWorld' or 'Verse'.")] string declaringNamespace = null,
        ...
        foreach (var typeDef in host.TypeSystem.MainModule.TypeDefinitions)
        {
            ...
            foreach (var method in typeDef.Methods)
            {
                ...
                if (parameterTypes != null)
                {
                    ...
                    {
                        if (!TypeMatches(method.Parameters[i].Type, parameterTypes[i]))
                        {
                            paramsOk = false;
                            break;
                        }
                ...
                if (!string.IsNullOrEmpty(returns) && !TypeMatches(method.ReturnType, returns))
                    continue;
    ...
    /// <summary>
    /// Match an <see cref="IType"/> against a user pattern. Generic-erasing —
    /// <c>List</c> matches any <c>List&lt;T&gt;</c>. Arrays matched via trailing <c>[]</c>.
    /// <c>Nullable&lt;T&gt;</c> is unwrapped so <c>int</c> matches <c>int?</c>.
    /// </summary>
    private static bool TypeMatches(IType type, string pattern)
    {
        if (string.IsNullOrEmpty(pattern)) return false;

        if (pattern.EndsWith("[]"))
        {
            return type is ArrayType arr && TypeMatches(arr.ElementType, pattern[..^2]);
        }
        if (type is ArrayType) return false;

        // ref / out / in surface as ByReferenceType — match the element
        var target = type;
        ...
        // Generic-erasing: List<Pawn> matches by GenericType ('List')
    ...
    private static string FormatResults(List<IMethod> matches, int limit)
    {
        ...
        for (int i = 0; i < emitting; i++)
        {
            ...
            sb.Append($"  {modifier}{m.DeclaringType.FullName}.{m.Name}" +
                      $"({parameters}) → {Formatter.FormatTypeRef(m.ReturnType)}\n");
        }
