...
# to the csproj publish target, dropped a new asset into Assets/, or the IL
...
if (-not (Test-Path $distDir)) {
    Write-Host "[SETUP] Build/dist/ not found. Run 'dotnet publish -c Release' first."
    exit 2
}
...
if (-not (Test-Path $allowlistPath)) {
    Write-Host "[SETUP] Source/Attribution/staged-allowlist.txt not found."
    exit 2
}
